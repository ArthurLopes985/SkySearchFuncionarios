...
public class PreencherPorId {
    public Funcionarios PreencherPorID(int id) throws ClassNotFoundException, SQLException {
    Connection con = ConnectionFactory.getConexao();
    PreparedStatement comando = con.prepareStatement("SELECT f.*, e.cep, e.rua, e.bairro, e.cidade FROM Funcionario f JOIN Endereco e ON f.endereco_id = e.id WHERE f.id = ?");
    comando.setInt(1, id);
